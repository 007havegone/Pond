...
//  printBDD(Start->dd);
  ...
  && (numStates = (int)(Cudd_CountMinterm(manager, Start->dd, 2 * num_alt_facts) - pow(2, num_alt_facts)))
  && numStates > 1){
    cout << "FO" << endl;
    ...
    if(!USE_CARD_GRP
    && (LUG_FOR == SPACE || LUG_FOR == INCREMENTAL)
    && HEURISTYPE != NONE
    && HEURISTYPE != CARD
    && num_lugs == 0){
      getHeuristic(NULL, NULL, 0);
...
void Search::graphToFile(const char* filename){
  ...
  graphFile << endl << "delimiter" << endl << setiosflags(ios::fixed) << setprecision(3);
    ...
    if(front->Expanded > 0){
      for(ActionNodeList::iterator act_it = front->NextActions->begin(); act_it != front->NextActions->end(); act_it++){
        ...
        for(StateDistribution *dist = action->NextState; dist != NULL; dist = dist->Next){
          ...
          graphFile << "OBS_" << obs->StateNo << endl << dist->Prob << endl;
